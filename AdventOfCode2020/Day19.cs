...
namespace AdventOfCode2020
{
    ...
    public sealed class Day19
    {
        ...
        {
            Assert.AreEqual(12, ValidCount(Day19SampleInput2));
        }

        private static long ValidCount(string[] input)
        {
            var ruleStrings = input.TakeWhile(i => i != "").ToArray();
            var messages = input.Skip(ruleStrings.Length + 1).ToArray();

            var cache = new Dictionary<(string, IRule), bool>();
            var rulesDictionary = new Dictionary<long, IRule>();
            foreach (var rule in ruleStrings.Select(ParseRule))
            {
            ...
            {
                var ruleParts = ruleString.Split(": ", 2);
                var number = long.Parse(ruleParts[0]);

                if (ruleParts[1].StartsWith("\""))
                {
                    return new TerminalRule(number, ruleParts[1][1..^1]);
                }

                var formats = ruleParts[1].Split('|').Select(f => f.Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(long.Parse).ToArray()).ToArray();
                return new NonTerminalRule(number, formats, rulesDictionary, cache);
            }
        }
        ...
        private sealed record NonTerminalRule(long Number, long[][] Formats, IDictionary<long, IRule> Rules, IDictionary<(string, IRule), bool> Cache) : IRule
        {
            private readonly Dictionary<string, bool> _knownMatches = new();

            public bool Matches(string message)
            {
                if (_knownMatches.TryGetValue(message, out var result))
                {
                    return result;
                }

                var matches = Formats.Any(f => MatchesFormat(message, f));
                _knownMatches.Add(message, matches);
                return matches;
            }

            private bool MatchesFormat(string message, long[] format)
            {
                if (message.Length < format.Length) return false;

                if (format.Length == 2)
                {
                    return Enumerable.Range(1, message.Length - 1)
                        .Select(i => (message[..i], message[i..]))
                        .Any(p => Rules[format[0]].Matches(p.Item1) && Rules[format[1]].Matches(p.Item2));
                }

                var remainingMessage = message;
                foreach (var rule in format)
                {
                    var match = FindMatch(remainingMessage, Rules[rule]);
                    if (match == null)
                    {
                        return false;
                    }

                    remainingMessage = remainingMessage[match.Length..];
                }

                return remainingMessage == "";

                string? FindMatch(string m, IRule r)
                {
                    for (var i = 1; i <= m.Length; i++)
                    {
                        var subMessage = m[..i];

                        if (Cache.TryGetValue((subMessage, r), out var v))
                        {
                            if (v)
                            {
                                return subMessage;
                        ...
                        if (r.Matches(subMessage))
                        {
                            Cache[(subMessage, r)] = true;
                            return subMessage;
                        }

                        Cache[(subMessage, r)] = false;
                    }
        ...
        private sealed record TerminalRule(long Number, string Terminal) : IRule
        {
            public bool Matches(string message) => Terminal == message;
        }

        private static readonly string[] Day19SampleInput1 =
        {
            "0: 4 1 5",
            "1: 2 3 | 3 2",
            "2: 4 4 | 5 5",
        ...
        private static readonly string[] Day19Input1 =
        {
            "62: 109 24 | 103 36",
            "9: 75 36 | 67 24",
            "66: 107 24 | 91 36",
        ...
        private static readonly string[] Day19Input2 =
        {
            "62: 109 24 | 103 36",
            "9: 75 36 | 67 24",
            "66: 107 24 | 91 36",

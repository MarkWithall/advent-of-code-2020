...
namespace AdventOfCode2020
{
    ...
    public class Day5
    {
        ...
        public void Part1Sample(string boardingPassDescription, int expectedRow, int expectedColumn, int expectedSeatId)
        {
            var boardingPass = BoardingPass.Create(boardingPassDescription);
            Assert.Multiple(() =>
            {
        ...
        {
            public static BoardingPass Create(string boardingPass)
            {
                var rowDescription = boardingPass[..7];
                var columnDescription = boardingPass[7..];
                var row = BinarySpacePartition(rowDescription, 'F', 'B');
                var column = BinarySpacePartition(columnDescription, 'L', 'R');
                return new BoardingPass(row, column);

                static int BinarySpacePartition(string str, char lower, char higher)
                {
                    var range = Enumerable.Range(0, (int) Math.Pow(2, str.Length)).ToArray();
                    foreach (var c in str)
                    {
                        if (c == lower)
                        {
                            range = range.Take(range.Length / 2).ToArray();
                        }
                        else if (c == higher)
                        {
                            range = range.Skip(range.Length / 2).ToArray();
                        }
                        else
                        {
